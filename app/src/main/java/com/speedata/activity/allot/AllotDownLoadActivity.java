...
public class AllotDownLoadActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if (v == tvStart) {
            ...
            downLoadInfors(HttpInterfaceModel.DownLoadAllotRecordItemCount,
                    HttpInterfaceModel.DownLoadAllotRecordItem);
        }
    }
...
//                                    handler.sendMessage(msg);
    ...
    List<AllotRecord> allotRecords = new ArrayList<AllotRecord>();
    ...
    private List<AllotRecord> downLoadOutRegister = new ArrayList<AllotRecord>();

    private void upDateDB() {
        allotUploadListAdapter = new AllotUploadListAdapter(mContext, downLoadOutRegister);
        listView.setAdapter(allotUploadListAdapter);
        allotRegisterDao.imDeleteAll();
        allotRegisterDao.imInsertList(downLoadOutRegister,false);
        List<AllotRecord> tempSum = allotRegisterDao.imRawQuery("select " +
                        "distinct BarCode" +
                        " from allotRecord ",
                null,
                AllotRecord.class);
        ...
        for (int i = 0; i < tempSum.size(); i++) {
            ...
            List<OutRegister> tempOutList =
                    outRegisterDao.imQueryList("BarCode=?", new
                            String[]{barcode});
            ...
            for (int m = 0; m < tempAllotList.size
                    (); m++) {
                count = count - tempAllotList.get
                        (m).getQuantity();
                logger.d("count-=" + count);
            }
            for (int z = 0; z < tempOutList.size
                    (); z++) {
                count = count - tempOutList.get
                        (z).getQuantity();
                logger.d("count-=" + count);
            }
            StoreBean store = tempAllotList.get(0);
