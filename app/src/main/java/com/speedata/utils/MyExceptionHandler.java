...
public class MyExceptionHandler implements UncaughtExceptionHandler {
    ...
    private boolean handleException(Thread thread, Throwable ex) {
        ...
        str = formatter.format(firstDate);
        // sb.append(startTimer);
        // sb.append("\n");
        sb.append(str); // 把当前的日期写入到字符串中
//		sb.append("来自" + Contants.app_version + "\n");
        sb.append("来自" + getVersionName(context) + "\n");
        Writer writer = new StringWriter();
        PrintWriter pw = new PrintWriter(writer);
        ex.printStackTrace(pw);
        errorresult = writer.toString();
        ...
        try {
            ...
            new Thread() {
                ...
                public void run() {
                    Looper.prepare();
                    LogSendEmail sender = new LogSendEmail();
                    sender.setFrom("speedatalog@sina.com");
                    sender.setTo("741183142@qq.com");
                    sender.setSubject("中铁二期梦龙错误报告 - " + str);
